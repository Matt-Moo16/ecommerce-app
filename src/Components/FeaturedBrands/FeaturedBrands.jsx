...
const FeaturedBrands = () => {
  return (
    <div style={{marginTop: '65px', backgroundColor: '#DFE0DF', display: 'flex', flexDirection: 'column'}}>
        <Typography variant='h4' sx={{fontFamily: 'Crimson Text', marginLeft: '10px', alignSelf: 'flex-start'}}>Men's Vintage Designer</Typography>
        <ImageList sx={{width: '100%', height: '600px', justifyContent: 'flex-end'}} cols={4} rows={2} variant='masonry'>
            {brandData.map((item, index) => (
                <ImageListItem key={index} sx={{marginTop: '25px', marginLeft: '10px'}}>
                <img
                    src={`${item.img}`}
                    alt={item.itemName}
                    loading="lazy"
        ...
        <Typography variant='h4' sx={{fontFamily: 'Crimson Text', marginRight: '10px', alignSelf: 'flex-end'}}>At prices you can afford! </Typography>
    </div>
  )
}
